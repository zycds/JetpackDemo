...
interface MediaContainerContact {
    ...
    class MediaContainerPresenter : IBasePresenter {
        ...
        fun registerObservableSdScan() {
            subscribe =
                RxBus.instance.toObservable(mMediaContainerView?.getActivityContext() as MediaActivity, MsgEvent::class.java)
                    ...
                    .subscribe({
                        ...
                        when (it.code) {
                            ScanSdReceiver.ACTION_MEDIA_SCANNER_FINISHED -> {
                                ...
                                Lg.debug(MediaContact.tag, "musicList size : ${ReadSdMedia.instance.getMusicLists()?.size}")
        ...
        fun <D : Media> getAdapter(): Adapter<D> {
            return Adapter()
        }

        inner class Adapter<D : Media> : BaseAdapter<ViewHolder, D>() {
            override fun itemCount(): Int {
                return ReadSdMedia.instance.getMusicLists()?.size!!
            }
